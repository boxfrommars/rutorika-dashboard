...
$(document).ready(function () {
  ...
  $('.upload-image-multiple-container .upload-result').magnificPopup({type: 'image'});
  ...
  $('.sortable-images')
    .on('click', '.thumbnail-image-destroy', function (e) {
      e.stopPropagation();

      var $thumb = $(this).parents('.thumbnail');
      var valueToRemove = $thumb.data('image');
      var $input = $(this).parents('.sortable-images').find('input');

      var currentValue = $input.val();
      var value = currentValue.replace(valueToRemove, '').replace('::', ':').replace(/(^:|:$)/g, '');
...
(function ($) {
  $(document).ready(function () {
    ...
    $('.js-uploader-multiple').each(function(){
      $(this).fileupload({
        dataType: 'json',
        url: $(this).data('url'),
        paramName: 'file',
        formData: [{
          ...
          /**
           * @type {{path:string, filename:string, success:boolean, errors:[]}} result
           */
          var result = data.result;
          if (result.success === false) {
            App.notify.validationError(result.errors);
            ...
            App.notify.success('Файл загружен успешно');
            var $container = $(this).parents('.upload-container');
            var currentValue = $container.find('.uploader-input').val();
            var value = currentValue ? currentValue + ':' + result.filename : result.filename;
            $container.find('.uploader-input').val(value);
            $container.trigger('uploaded', [result]);
          }
        }
      })
    ...
    $('.upload-image-multiple-container').on('uploaded', function(e, result) {
      console.log(result);

      var $thumb = $('<div>').data('image', result.filename).addClass('thumbnail pull-left');
      var $image = $('<a>').attr('href', result.path).addClass('thumb upload-result').css('background-image', 'url(' + result.path + ')');
      var $delLink = $('<a>').attr('href', '#').addClass('thumbnail-link thumbnail-image-destroy').append('<i class="glyphicon glyphicon-remove"></i>');

      $thumb.append($image);
      $thumb.append($delLink);

      $thumb.appendTo($(this).find('.sortable-images'));
    });
